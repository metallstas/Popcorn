...
void AsBorder::Draw(HDC hdc,RECT &paint_area) {
   ...
   for (i = 0; i < 50; i++)
      Draw_Elemet(hdc, 2, 1 + i * 4, false);

   for (i = 0; i < 50; i++)
      Draw_Elemet(hdc, 201, 1 + i * 4, false);

   //Рисуем верхнюю рамку

   for (i = 0; i < 50; i++)
      Draw_Elemet(hdc, 3 + i * 4, 0, true);
}

//------------------------------------------------------------------------------------------------------------

void AsBorder::Draw_Elemet(HDC hdc, int x, int y, bool top_border) {
   ...
   if (top_border)
      Rectangle(hdc, x * AsConfig::Global_Scale, (y + 1) * AsConfig::Global_Scale, (x + 4) * AsConfig::Global_Scale, (4 + y) * AsConfig::Global_Scale);
   else
      Rectangle(hdc, (x + 1) * AsConfig::Global_Scale, y * AsConfig::Global_Scale, (x + 4) * AsConfig::Global_Scale, (4 + y) * AsConfig::Global_Scale);
